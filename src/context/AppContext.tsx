...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const [promoBanners, setPromoBanners] = useState<PromoBanner[]>(() => 
    getStored('bali_tour_promo_banners', [])
  );
  ...
  useEffect(() => {
    ...
    checkAndSeed().then(() => {
      ...
      unsubscribers.push(syncCollection('promo_banners', setPromoBanners));
  ...
  useEffect(() => {
    safeSetItem('bali_tour_promo_banners', JSON.stringify(promoBanners));
  }, [promoBanners]);
  ...
  const addPromoBanner = async (banner: Omit<PromoBanner, 'id'>) => {
    ...
    try {
      await setDoc(doc(db, 'promo_banners', newItem.id), cleanObj(newItem));
    } catch (err) {
      console.error('Error adding promo banner:', err);
    }
  };
  const updatePromoBanner = async (banner: PromoBanner) => {
    try {
      await setDoc(doc(db, 'promo_banners', banner.id), cleanObj(banner));
    } catch (err) {
