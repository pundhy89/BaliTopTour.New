    ...
    <div className="flex-1 pb-24 overflow-y-auto">
      ...
      <div className="relative w-full aspect-[1/1] overflow-hidden">
        ...
        <div className="absolute top-6 left-6 right-6 flex justify-between items-start z-10">
          <div>
            <p className="text-white/90 text-sm font-semibold flex items-center gap-1 drop-shadow-md [text-shadow:0px_0px_3px_rgba(0,0,0,0.8)]">
              Hi, {userName || 'Pp'}! <span className="animate-bounce">👋</span>
            </p>
            <h2 className="text-white text-2xl font-bold tracking-tight mt-1 flex items-center gap-1.5 drop-shadow-md [text-shadow:0px_0px_3px_rgba(0,0,0,0.8)]">
          ...
          <div className="bg-black/70 backdrop-blur-md border border-white/10 rounded-full p-0.5 flex items-center gap-0.5 shadow-md">
            <button
              onClick={() => {
                setLanguage('en');
                trackAction('lang_changed_en', 'Language changed to English');
              }}
              className={`text-[8px] font-bold px-2 py-0.5 rounded-full transition-all duration-300 ${
                language === 'en'
      ...
      {/* Promo Banner Slide moved to bottom */}
      ...
      {promoBanners && promoBanners.length > 0 && (
        <div className="px-5 mt-6 mb-6 relative">
          <div className="relative aspect-[2.2/1] rounded-[24px] overflow-hidden shadow-sm">
            {promoBanners.map((promo, idx) => (
              <div 
                key={promo.id}
                className={`absolute inset-0 transition-opacity duration-1000 flex flex-col justify-center items-start p-6 ${
                  idx === currentPromoIndex ? 'opacity-100 z-10' : 'opacity-0 z-0'
                }`}
              >
                <img 
                  src={promo.image_url} 
                ...
                <div className="relative z-20 flex flex-col items-start text-left w-full">
                  {promo.subtitle && (
                    <p className="text-yellow-400 font-extrabold text-[10px] tracking-[0.2em] mb-1 drop-shadow-md">
                      {promo.subtitle}
                    </p>
                  )}
                  <h3 className="text-white font-black text-2xl tracking-tight leading-none mb-1 drop-shadow-md">
                    {promo.title}
                  </h3>
                  {promo.description && (
                    <p className="text-white/90 text-xs font-semibold mb-3 max-w-[80%] drop-shadow-sm">
                      {promo.description}
                    </p>
                  )}
                  {promo.button_text && (
                    <button className="bg-white text-slate-800 hover:text-indigo-600 hover:scale-105 active:scale-95 transition-all px-4 py-2 rounded-full text-xs font-extrabold flex items-center gap-1 shadow-md">
                      {promo.button_text} <span className="font-bold">&gt;</span>
                    </button>
                  )}
                </div>
              </div>
            ...
            {promoBanners.length > 1 && (
              <div className="absolute bottom-3 left-0 right-0 flex justify-center items-center gap-1.5 z-30">
