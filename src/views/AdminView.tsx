...
export default function AdminView({ navigate }: { navigate: (to: string | number) => void }) {
  ...
  const [apPricesList, setApPricesList] = useState<{ id?: string, keterangan: string, harga: number }[]>([{ keterangan: 'Dewasa', harga: 100000 }]);
  const [apUnit, setApUnit] = useState('/ Pax');
  ...
  const [actPinNumber, setActPinNumber] = useState<number | ''>('');
  const [actPinColor, setActPinColor] = useState<string>('blue');
  const [actMapX, setActMapX] = useState<number>(50);
  const [actMapY, setActMapY] = useState<number>(50);
  ...
  const handleMapPointerUpOrLeave = (e: React.MouseEvent<HTMLDivElement>) => {
  ...
  const handleSaveActivity = async (e: React.FormEvent) => {
    ...
    if (editingAct) {
      const updated: Activity = {
        ...
        pin_number: actPinNumber === '' ? undefined : actPinNumber,
        pin_color: actPinColor,
        map_x: actMapX,
        map_y: actMapY,
      ...
      <div className="px-5 mt-4">
            ...
            {!showPkgForm ? (
              ...
              {editingPkg ? (
                      ...
                      {tourPackageOptions.filter(o => o.package_id === editingPkg.id).map(opt => (
                        <div key={opt.id} className="bg-white border border-slate-100 rounded-2xl p-3 flex items-center justify-between gap-1 shadow-xs">
                          <div className="min-w-0 flex-1">
                            ...
                            {(opt.price_idr ?? 0) > 0 && (
                              <p className="text-indigo-600 text-[10px] font-extrabold mt-0.5">
                                Rp {opt.price_idr?.toLocaleString('id-ID')}
                              </p>
                            )}
                          </div>
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          onClick={async () => {
                            ...
                            setIsTranslating(true);
                            let trans = { en: [optName, optDesc], zh: [optName, optDesc], id: [optName, optDesc] };
                            try {
                              trans = await translateTexts([optName, optDesc]);
                            } catch (e) {
                              console.error(e);
                            } finally {
                              setIsTranslating(false);
                            ...
                            if (optEditId) {
                              ...
                              updateTourPackageOption({
                                ...
                                name: optName.trim(),
                                name_id: optName.trim(),
                                name_en: trans.en[0] || optName.trim(),
                                name_zh: trans.zh[0] || optName.trim(),
                                description: optDesc.trim(),
                                description_id: optDesc.trim(),
                                description_en: trans.en[1] || optDesc.trim(),
                                description_zh: trans.zh[1] || optDesc.trim(),
                                price_idr: optPrice,
                          ...
                          disabled={isTranslating}
                          className="flex-1 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-extrabold py-2 rounded-xl text-[10px] uppercase tracking-wider"
                        >
                ...
                <div className="mt-6 pt-6 border-t border-slate-200">
                  <p className="text-[10px] text-center text-slate-500 font-bold bg-slate-100 p-4 rounded-2xl border border-dashed border-slate-200">
                    ℹ️ Silakan <strong>Simpan</strong> paket tour terlebih dahulu untuk bisa menambahkan <strong>Opsi Tour</strong> dan <strong>Titik Pemberhentian</strong>.
                  </p>
                </div>
              )}
              </>
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => {
                            ...
                            setActPinNumber(act.pin_number !== undefined ? act.pin_number : '');
                            setActPinColor(act.pin_color || 'blue');
                            setActMapX(act.map_x !== undefined ? act.map_x : 50);
                            setActMapY(act.map_y !== undefined ? act.map_y : 50);
                ...
                <div className="bg-slate-50 border border-slate-150 p-3.5 rounded-2xl flex flex-col gap-3">
                  ...
                  <div className="relative flex flex-col items-center">
                    ...
                    >
                      ...
                      <div 
                        style={{ left: `${actMapX}%`, top: `${actMapY}%` }}
                        className="absolute -translate-x-1/2 -translate-y-full pointer-events-none transition-all duration-75"
                      >
                        <div className="flex flex-col items-center">
                          <div className={`text-white font-bold text-[8px] px-1.5 py-0.5 rounded-full shadow-md whitespace-nowrap mb-0.5 animate-bounce ${
                            actPinColor === 'blue' ? 'bg-blue-500' :
                            actPinColor === 'teal' ? 'bg-teal-500' :
                            actPinColor === 'orange' ? 'bg-orange-500' :
                            actPinColor === 'pink' ? 'bg-pink-500' :
                            actPinColor === 'purple' ? 'bg-purple-500' :
                            'bg-red-600'
                          }`}>
                            {actPinNumber !== '' ? actPinNumber : (language === 'zh' ? '活动地点' : language === 'en' ? 'Activity Location' : 'Titik Lokasi')}
                          </div>
                          <MapPin size={24} className={`filter drop-shadow-md ${
                            actPinColor === 'blue' ? 'text-blue-600 fill-blue-100' :
                            actPinColor === 'teal' ? 'text-teal-600 fill-teal-100' :
                            actPinColor === 'orange' ? 'text-orange-600 fill-orange-100' :
                            actPinColor === 'pink' ? 'text-pink-600 fill-pink-100' :
                            actPinColor === 'purple' ? 'text-purple-600 fill-purple-100' :
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-2">
                    <div>
                      ...
                      <input
                        type="number"
                        value={actPinNumber}
                        onChange={(e) => setActPinNumber(e.target.value === '' ? '' : Number(e.target.value))}
                        placeholder="e.g. 1"
                        className="w-full bg-slate-50 border border-slate-200 outline-none text-xs font-semibold text-slate-800 rounded-xl py-2 px-3 focus:border-indigo-500 focus:bg-white"
                      />
                    </div>
                    <div>
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">
                      ...
                      <select
                        value={actPinColor}
                        onChange={(e) => setActPinColor(e.target.value)}
                        className="w-full bg-slate-50 border border-slate-200 outline-none text-xs font-semibold text-slate-800 rounded-xl py-2 px-3 focus:border-indigo-500 focus:bg-white"
                      >
                        <option value="blue">Biru (Blue)</option>
                        <option value="teal">Tosca (Teal)</option>
                ...
                <div className="mt-6 pt-6 border-t border-slate-200 flex flex-col gap-6 text-left">
                  ...
                  <div className="bg-slate-50 border border-slate-150 rounded-[28px] p-4.5">
                            ...
                            <div className="flex gap-1.5">
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  const packagePrices = activityPackagePrices.filter(pr => pr.activity_package_id === ap.id);
                                  if (packagePrices.length > 0) {
                                      setApPricesList(packagePrices.map(pr => ({ id: pr.id, keterangan: pr.label_id || pr.label, harga: pr.price_idr })));
                                  } else {
                                      setApPricesList([{ keterangan: 'Dewasa', harga: 0 }]);
                                  }
                                  setApUnit('/ Pax');
                                }}
                    ...
                    <div className="bg-white border border-dashed border-slate-200 rounded-2xl p-3.5 flex flex-col gap-2.5">
                      ...
                      <div className="grid grid-cols-2 gap-2.5">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            placeholder="cth: / Pax"
                            value={apUnit}
                            onChange={e => setApUnit(e.target.value)}
                            className="w-full bg-slate-50 border border-slate-200 outline-none text-xs font-semibold text-slate-800 rounded-xl py-2 px-3"
                          />
                        </div>
                      </div>
                      ...
                      {/* Prices List */}
                      <div className="border border-slate-200 rounded-xl p-3 flex flex-col gap-2 mt-1">
                        <label className="text-[9px] font-black text-slate-600 uppercase tracking-widest">Daftar Harga Paket</label>
                        {apPricesList.map((item, idx) => (
                          <div key={idx} className="flex gap-2 items-center">
                            <input
                              type="text"
                              placeholder="Keterangan (cth: Dewasa)"
                              value={item.keterangan}
                              onChange={e => {
                                const newL = [...apPricesList];
                                newL[idx].keterangan = e.target.value;
                                setApPricesList(newL);
                              }}
                              className="flex-1 bg-white border border-slate-200 outline-none text-[10px] font-semibold text-slate-800 rounded-lg py-2 px-2"
                            />
                            <input
                              type="number"
                              placeholder="Harga (IDR)"
                              ...
                              onChange={e => {
                                const newL = [...apPricesList];
                                newL[idx].harga = Number(e.target.value);
                                setApPricesList(newL);
                              }}
                              className="w-[100px] bg-white border border-slate-200 outline-none text-[10px] font-semibold text-slate-800 rounded-lg py-2 px-2"
                            />
                            {apPricesList.length > 1 && (
                              <button
                                type="button"
                                onClick={() => {
                                  setApPricesList(apPricesList.filter((_, i) => i !== idx));
                                }}
        ...
        {activeTab === 'gallery' && (
          <div className="flex flex-col gap-6">
                ...
                {activityGallery.map(img => (
                  <div key={img.id} className="relative aspect-square rounded-xl overflow-hidden group">
                    ...
                    <button
                      ...
                      className="absolute inset-0 bg-black/60 opacity-100 md:opacity-0 group-hover:opacity-100 text-white flex items-center justify-center transition-opacity"
                    >
                      <Trash2 size={16} />
                    </button>
                  </div>
            ...
            {/* Promo Slider Banner Addition */}
            <div className="bg-white rounded-3xl border border-slate-100 p-5 shadow-sm">
              <h3 className="text-slate-800 font-extrabold text-xs mb-3 flex items-center gap-1.5">
                <Layers size={15} className="text-slate-500" />
                Tambah Banner Promo Slide
              </h3>
              <div className="flex flex-col gap-3">
                <ImageUploadOrUrl
                ...
                <div>
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1">Sub Judul (cth: COMING SOON)</label>
                  <input
                    type="text"
                    value={promoSubtitle}
                ...
                <div>
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1">Judul Utama (cth: Villa & Hotel)</label>
                  <input
                    type="text"
                    value={promoTitle}
                ...
                <div>
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1">Deskripsi Singkat</label>
                  <input
                    type="text"
                    value={promoDesc}
                ...
                <div>
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1">Teks Tombol</label>
                  <input
                    type="text"
                    value={promoBtnText}
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => {
                      if (promoBannerUrl.trim()) {
                        if (editingPromoId) {
                          updatePromoBanner({
                            id: editingPromoId,
                            image_url: promoBannerUrl.trim(),
                            subtitle: promoSubtitle.trim(),
                            title: promoTitle.trim(),
                            description: promoDesc.trim(),
                            button_text: promoBtnText.trim() || 'Notify me >',
                            sort_order: promoBanners.find(p => p.id === editingPromoId)?.sort_order ?? 0
                          });
                          ...
                          addPromoBanner({
                            ...
                            description: promoDesc.trim(),
                            button_text: promoBtnText.trim() || 'Notify me >',
                            sort_order: promoBanners.length
                          });
                        }
                        setPromoBannerUrl('');
                        setPromoSubtitle('');
                    ...
                    className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white text-[11px] font-extrabold py-2.5 rounded-2xl cursor-pointer active:scale-95 transition-all text-center uppercase tracking-wider"
                  >
                    {editingPromoId ? 'Simpan Perubahan' : 'Tambah Promo Banner'}
                  </button>
                  {editingPromoId && (
                    <button
                      onClick={() => {
                      ...
                      className="bg-slate-200 hover:bg-slate-300 text-slate-600 text-[11px] font-extrabold py-2.5 px-4 rounded-2xl cursor-pointer active:scale-95 transition-all text-center uppercase tracking-wider"
                    >
                      Batal
                    </button>
                  )}
              ...
              <div className="grid grid-cols-2 gap-2 mt-4 max-h-[200px] overflow-y-auto">
                {promoBanners.map(b => (
                  <div key={b.id} className="relative aspect-[1.25/1] bg-slate-50 rounded-xl overflow-hidden group border border-slate-100 flex flex-col justify-end p-2">
                    <img src={b.image_url} className="absolute inset-0 w-full h-full object-cover z-0" />
                    <div className="absolute inset-0 bg-black/40 z-10" />
                    
                    <div className="relative z-20">
                      <p className="text-[8px] text-yellow-300 font-bold uppercase">{b.subtitle}</p>
                      <h4 className="text-white text-xs font-bold leading-tight">{b.title}</h4>
                      <p className="text-white/80 text-[8px]">{b.description}</p>
                    </div>

                    <div className="absolute top-2 right-2 z-30 flex gap-1 opacity-100 transition-opacity">
                      <button
                        onClick={() => {
                          setEditingPromoId(b.id);
                          setPromoBannerUrl(b.image_url);
                          setPromoSubtitle(b.subtitle || '');
                          setPromoTitle(b.title || '');
                          setPromoDesc(b.description || '');
                          setPromoBtnText(b.button_text || '');
                        }}
                        className="bg-black/60 p-1.5 rounded-lg text-white flex items-center justify-center hover:bg-indigo-500"
                      >
                      ...
                      <button
                        ...
                        className="bg-black/60 p-1.5 rounded-lg text-white flex items-center justify-center hover:bg-red-500"
                      >
                        <Trash2 size={12} />
                      </button>
                    </div>
