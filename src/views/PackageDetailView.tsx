...
export default function PackageDetailView({ id, navigate }: { id: string; navigate: (to: string | number) => void }) {
  ...
  const activeOpt = options.find(o => o.id === activeOptId);
  const displayPrice = (activeOpt && activeOpt.price_idr && activeOpt.price_idr > 0) ? activeOpt.price_idr : pkg.price_idr;
